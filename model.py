...
def train_model(X_train, y_train):
    ...
    voting_model = VotingClassifier(estimators=[('rf', rf), ('dt', dt), ('et', et)], voting='hard', n_jobs=-1, verbose=True)
...
if __name__ == '__main__':
    ...
    if df is not None:
        ...
        X_train, X_test, y_train, y_test = train_test_split(df[selected_features], df[' Label'], test_size=TEST_SIZE, random_state=RANDOM_STATE, stratify=df[' Label'])
        voting_model = train_model(X_train, y_train)
        evaluate_model(voting_model, X_test, y_test, le)
